...
$venvCount = @($zip.Entries | Where-Object { $_.FullName -match '(^|[\\/])\.venv[\\/]' }).Count
$setupCount = @($zip.Entries | Where-Object { $_.FullName -match '(^|[\\/])setup\.ps1$' }).Count
$runCount = @($zip.Entries | Where-Object { $_.FullName -match '(^|[\\/])run\.ps1$' }).Count
$zip.Dispose()
...
if ($setupCount -ne 1 -or $runCount -ne 1) {
    Write-Host "警告：压缩包缺少 setup.ps1 或 run.ps1，请检查打包内容" -ForegroundColor Red
    exit 1
}
